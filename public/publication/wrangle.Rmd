...
## **Data Cleanup**
Examine the combined data frame with `str`; check that you columns corresponding to u.data, u.user, and u.item columns. One thing to notice is that types are off for a couple variables. Specifically, *timestamp* is shown to be an uninterpretable integer and *release date* is stored as character. The other is that *video release date* has nothing but missing values.

6. *timestamp* stores datetime type incorrectly as integers and *release date* stores a combination of year, month and day as cahracter. Use appropriate type converting function from **lubridate** package to transform the variable types. Confirm that we have data somewhere around 1997 - 1998 with `range(df$timestamp)`, substituting your name for the final data.
...
8. Note that some column names in your combined dataset actually stand for genres. We want to obtain a data frame containing two variables: genres and their count. Combine `gather` from **tidyr** and `distinct`, `filter`, `count` functions from **dplyr** to accomplish this. Which genre was least and which genre was most produced?
...
10. Now make a horizontally pointed bar chart of counts by genre. Additionally, make the annotations nicer with function  
```
...
12. Create a grid of plots showing bar plots signifying proportions of ratings given to a certain genre of movie by a user of a certain occupation. This is a large plot that is legible only when you stretch the pane with *Plots* tab throughout your screen and may take a few moments to render.  
  ...
  Second, create the basic bar plots with   
```
facet_grid(genre ~ occupation)
```  
