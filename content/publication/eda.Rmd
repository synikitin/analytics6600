...
- After correcting names, we need to also fix data types. Output of `str(df)` shows that, say **budget_2013$**, has type `chr` which represents text data. Use `mutate` and `as.double` to convert all variables that are by their nature numerical, but are stored as text.
...
- Next, select international only gross and clean_test variables followed by creating a new variable with a single value of "international". Save this as a separate data frame. Repeat the same computations to create a data frame for "domestic", and then combine the two into a new data frame using `bind_rows` (here you had to name the new variables with the same name, say mrket). Calculate median gross sales for combination of clean_test result and origin of sales by using both variables for grouping. What can you say about relation of passing the test and market on gross sales?
